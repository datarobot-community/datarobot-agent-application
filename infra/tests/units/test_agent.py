...
class TestA2AEndpointRuntimeParameter:
    """Guard against silent A2A breakage: the template must inject AGENT_A2A_ENDPOINT
    into agent runtime parameters when both dragent and A2A are enabled."""
    ...
    def test_a2a_endpoint_param_present_when_a2a_and_dragent_enabled(
        ...
        AGENT_A2A_ENDPOINT must appear in agent_agent_runtime_parameters."""
    ...
    def test_a2a_endpoint_param_absent_when_a2a_disabled(self, monkeypatch, tmp_path):
        """When workflow.yaml has no A2A config, AGENT_A2A_ENDPOINT must NOT
