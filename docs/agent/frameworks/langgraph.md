...
The generated `workflow.yaml` for the LangGraph agent looks like the following:
...
When using the DRAgent front server, NAT resolves additional tools (for example, A2A remote agents) from `workflow.yaml` `tool_names`, merges them with MCP tools, and passes the combined set to `MyAgent`.

### Custom local tools

To add a custom tool, define a plain Python function and wrap it with the `@tool` decorator from LangChain or `langchain_core.tools.tool()`. Then pass it to the agent nodes inside `graph_factory`:
...
The `tools` parameter in `graph_factory` contains all MCP and workflow tools. Prepend custom tools to it so each node has access to both custom and runtime-injected tools.

Use the `BaseTool` class from LangChain for tools with more complex input schemas:
...
The `prompt_template` controls how user input is structured before it reaches the graph. It is a `ChatPromptTemplate` defined at module level in `myagent.py`:
...
- `{topic}`&mdash;the user input text (extracted from the prompt).

To change how user input is presented to the agent, modify this template. Add more context, change the system message, or introduce additional variables. The variable names must match the keys returned by the `kickoff_inputs` mapping in the factory.
...
`make_system_prompt()` is a helper from `datarobot_genai` that wraps the prompt with consistent formatting. Always use it instead of passing raw strings.

To modify agent behavior:

- Update the system prompt for a node&mdash;change the string passed to `make_system_prompt()` inside `graph_factory`.
...
- Add nodes&mdash;create a `create_agent()` call with its own system prompt and wire it into the graph with `add_node()` / `add_edge()`.

### Tips

Consider the following tips when modifying prompts.
...
The base `LangGraphAgent` class enables chunk-level streaming with `stream=True`.
