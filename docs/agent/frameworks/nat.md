...
The NAT (NVIDIA NeMo Agent Toolkit) agent uses a YAML-first configuration to define agents, tools, and workflows. Orchestration and built-in function types are declared in `workflow.yaml`; [custom local tools](#custom-local-tools) also use short Python in `register.py` (`nat_tool`).

## NAT `workflow.yaml` requirements (read this first)

NAT validates `workflow.yaml` with a fixed schema. Other parts of this repository document LangGraph-oriented patterns; those are not valid inside the NAT `functions` block unless a NAT discriminator explicitly supports them.

1. Do not use `python_function` in `functions`. Some examples in the ecosystem use `_type: python_function` with `module_path` and `function_name` to point at Python callables. That is not a valid [NAT function discriminator](https://docs.nvidia.com/nemo/agent-toolkit/index.html). This produces errors such as: `Input tag 'python_function' found using discriminator() does not match any of the expected tags`. For custom code, use `nat_tool` in `register.py` plus a matching `functions` entry (see [Custom local tools](#custom-local-tools)) or only [built-in NAT `/_type` values](https://docs.nvidia.com/nemo/agent-toolkit/index.html).

2. Every name in `workflow.tool_names` must exist as a key under `functions:` and/or `function_groups:`. Adding `nat_tool(my_fn, "my_tool", ...)` in `register.py` without a matching `functions.my_tool` entry in `workflow.yaml` produces: `ValueError: Function 'my_tool' not found in list of functions`. Python registration alone does not create the YAML entry the NeMo builder loads.
...
4. Prefer `typing.Annotated` on parameters for custom `nat_tool` functions so the LLM receives clear argument descriptions (see [Custom local tools](#custom-local-tools)).

5. Do not combine `from __future__ import annotations` with `dict[str, Any]` return types on multi-argument tools. NAT wraps tools that have more than one parameter in an internal pydantic adapter inside the `function_info` module in NeMo. Deferred annotations such as `-> dict[str, Any]` are re-evaluated there, where bare `Any` is not in scope, which surfaces at runtime as `NameError: name 'Any' is not defined` while building `web_research` (or the first custom function). Prefer plain annotations without `__future__` annotations, or return a concrete type / `dict[str, object]` instead of `dict[str, Any]`.

6. `register.py` must be imported at startup when it contains `nat_tool(...)` calls. The `nat_tool` helper registers the tool with the workflow registry in NeMo when the module loads. Generated NAT projects ship a stub `register.py` and `import agent.register  # noqa: F401` in `myagent.py` so adding `nat_tool` later does not require wiring an additional import. If you remove that import, add it back (or import `register` at the top of `agent/__init__.py` *before* `from agent.myagent import ...`) before relying on custom tools; otherwise the runtime can fail when resolving tool implementations.
...
NeMo builds the callable list from the `functions` (and `function_groups`) section of `workflow.yaml`. `nat_tool` in Python does not remove the need for YAML. For each name passed as the second argument to `nat_tool`, add a matching `functions` entry; otherwise the build fails with `ValueError: Function '…' not found in list of functions`.

Use this table when adding, renaming, or removing a custom tool:
...
| 2 | Add a name to `workflow.tool_names`. | It must be a `functions` key, a `function_groups` key (for example, `mcp_tools`), or it will fail at runtime. |
...
Define multiple LLMs and assign them to different functions.
...
The template ships with a `word_counter` tool that counts words in a given text. It is defined in `register.py`, registered via `nat_tool`, and declared in `workflow.yaml`. Remove it or replace it with custom tools.
...
Step 1&mdash;Define the tool function (for example, in `agent/agent/tools.py`):
...
Sub-agents can also be defined as tools using `_type: chat_completion` with a `system_prompt`&mdash;these are built-in function types in NAT that act as callable tools for the orchestrator agent.
...
NAT agents define all prompts declaratively in `workflow.yaml`. Configure two levels: function system prompts (sub-agent behavior) and the workflow system prompt (orchestrator behavior).
...
The `workflow` section has its own `system_prompt` that controls the orchestrator&mdash;the top-level agent that determines which functions to call:
...
Modify NAT agent behavior in the following ways:
...
- Assign different LLMs&mdash;set `llm_name` per function to use different models for different tasks (for example, a fast model for planning, a capable model for writing).
- Add functions&mdash;create an entry in `functions` with `_type: chat_completion`, a `system_prompt`, and a `description`. Then add it to `workflow.tool_names`.
- Change function descriptions&mdash;update `description` to influence when the orchestrator calls a function.

### Tips

Consider the following tips when configuring NAT prompts.
...
- Use `description` on functions to help the orchestrator determine when to call each function.
...
Choose the NAT framework in the following cases.

- Defining agents without writing Python orchestration code.
- Iterating on agent prompts and tool configurations through YAML.
- Using NAT-native features like built-in tool types, authentication providers, and multi-LLM configurations.
