...
Migrating to 11.8.8 introduces the following changes to the agent layout:
...
Complete the following steps to migrate an agent to the 11.8.8 layout.

### 1. Update imports

Update the module imports as follows.
...
### 2. Move the `prompt_template` property to module level

Move the `prompt_template` property to a module-level constant.
...
Extract the `workflow` property and agent node properties into a standalone factory function. The function receives `llm`, `tools`, and `verbose` as arguments instead of reading them from `self`.
...
Key differences:
- `self.llm()` → `llm` (passed as argument)
- `self.tools` → `tools` (passed as argument)
- `self.verbose` → `verbose` (passed as argument, forwarded as `debug=verbose`)
...
### 6. Update the `custompy_adaptor` function

Update `custompy_adaptor` to build the LLM via `get_llm()` and load MCP tools inline.
