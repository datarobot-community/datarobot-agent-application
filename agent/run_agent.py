...
if (
    ...
    os.environ["PATH"] = (
        str(Path(_VENV_DIR) / "bin") + os.pathsep + os.environ.get("PATH", "")
    )
    os.environ["VIRTUAL_ENV"] = _VENV_DIR

# ruff: noqa: E402 -- the imports below must stay after the venv bootstrap above:
# they resolve against the agent's own venv, which only exists once `uv sync` has run.
